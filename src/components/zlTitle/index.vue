<template>
    <!-- 标题组件 -->
    <div class="zltitle_box" :class="props.showHover ? 'showHover' : ''"
        :style="`border-bottom: ${getBottomLine};cursor: ${getActive};padding: ${props.padding};`">
        <div v-if="props.dot" class="zltitle_dot" :style="`background-color: ${getColor};`"></div>
        <div class="zltitle_content"
            :style="`color: ${getFontColor};font-size: ${getFontSize};font-weight: ${getFontWeight}`">
            <slot></slot>
        </div>
...
const props = defineProps({
    dot: { type: Boolean, default: true },     // 是否显示小圆点
    type: { // 颜色类型
        type: String,
        default: () => 'primary',
    },
...
const getFontColor = computed<string>(() => {
    return props.fontColor ? props.fontColor : variables.mainFontColor
})
// 设置字体大小
