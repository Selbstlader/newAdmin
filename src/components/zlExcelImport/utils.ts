...
const getSerialNo = (err:string|undefined, warning: string|undefined, index: number) => {
    if (err) return (index + 1).toString() + ' ' + err
    if (warning) return (index + 1).toString() + ' ' + warning
    return (index + 1).toString()
}

export { download, getSerialNo  }