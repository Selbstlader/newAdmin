<template>
    <div class="zl_vxe_table" :style="props.haveMenu ? `border-radius: 0`:''">
        <!-- 搜索区域 start -->
        <div v-if="props.showSearch" class="z_header" :style="props.haveMenu ? `border-radius: 0 `:''" :class="{ z_header_position: props.topPosition && table.unfold }">
        ...
        <div v-if="props.tools" class="zl_vxe_top" :class="{zl_vxe_top_position: props.topPosition && table.unfold }">
            ...
            <div class="zl_top_r">
                ...
                <!-- 导入 -->
                <div v-if="props.import"><ZlTag style="margin-right: 12px;" :disabled="props.importDisabled" tagType="button" type="search" effect="light" size="small" @click="bindImportTable"><el-icon style="margin-right: 6px;"><Upload /></el-icon>导入</ZlTag></div>
                <!-- 导出 -->
                <div v-if="props.export"><ZlTag style="margin-right: 12px;" :disabled="props.exportDisabled" tagType="button" type="search" effect="light" size="small" @click="bindExportTable"><el-icon style="margin-right: 6px;"><Download /></el-icon>导出</ZlTag></div>
                <!-- 表格列设置 -->
        ...
        <div class="zl_vxe_bottom" :style="!props.pager?'padding-bottom: 12px;': ''" :class="{zl_vxe_bottom_position: props.topPosition && table.unfold }">
            ...
            <div v-if="props.showTable && table.showTable" class="table_box">
                <vxe-grid ref="xGrid2" height="100%" border='inner' v-bind="gridOptions2" :loading='table.loading' stripe
                    auto-resize show-overflow 
                    highlight-hover-row 
                    ...
                    @cell-click="clickRow">
                    <!-- 单元格插槽 -->
                    <template v-for="(item, index) in props.zlvalue.columns.filter((e: any) => e.slots && e.slots.default)" :key="index"
            ...
            <div v-if="props.pager" class="zl_pager">
                <el-pagination  @size-change="sizeChange" @current-change="currentChange" :id="`${props.tableID}pagerID`"
                v-model:currentPage="tablePage" :page-sizes="pageSize" :page-size="Number(table.search.pageSize)"
...
console.log('props.zlvalue.columns.filter((e: any) => !e.noShow)', props.zlvalue.columns.filter((e: any) => !e.noShow))
...
const init = async () => {
    ...
    if (props.getTableColumns && props.custom) columns = await toVxeTable(table.userId+'' || '', props.zlvalue.pageName + '_' + props.zlvalue.tableName, props.zlvalue.columns);
    ...
    props.zlvalue.columns.forEach((item: ZlVXETableColumn) => {
        if (item.searchType) {
            ...
            if (item.searchType == 'checkbox' && item.options && item.options.length > 0) {
                ...
                indeterminate.value[item.field] = table.searchs[item.field].checkbox!.length > 0 && table.searchs[item.field].checkbox!.length < item.options.length;
            }
            // 得到搜索的字段
...
const itemSearch = (item: any, type: boolean) => {
    ...
    if (type) {
        ...
        if (item.searchType === 'range') {
            param.value = (table.searchs[item!.field!].minNumber ? table.searchs[item!.field!].minNumber : item.toNumber ? 0 : '') + ',' + (table.searchs[item!.field!].maxNumber ? table.searchs[item!.field!].maxNumber : item.toNumber ? 0 : '')
        }
        if (item.searchType === 'checkbox' && table.searchs[item!.field!].checkbox!.length > 0) {
            param.value = table.searchs[item!.field!].checkbox!.length == checkboxNumber[item!.field!] ? '' : table.searchs[item!.field!].checkbox!.toString()
        }
...
const handleCheckAllChange = (e: boolean, item: any) => {
    ...
    indeterminate.value[item.slot] = table.searchs[item!.field!].checkbox!.length > 0 && table.searchs[item!.field!].checkbox!.length < item.data.length
}
...
const toolbarCustomEvent: VxeTableEvents.Custom = async (params: any) => {
    ...
    switch (params.type) {
        case 'confirm': gridOptions2.columns = await toVxeTable(table.userId+'' || '', table.pageName + '_' + table.tableName, table.columns, visibleColumn);break;
        case 'reset': gridOptions2.columns = await toVxeTable(table.userId+'' || '', table.pageName + '_' + table.tableName, table.columns, table.columns, true);break;
        case 'close': break;
    }
}
// 列宽拖动监听
...
const PostQueryTableConfig = (param: vxeTableOrder) => {   
    ...
    return request({
        url: Api.Host + `/system/tableconfig/query/config/${param.userId}`,
        method: 'get',
        params: req
    });
